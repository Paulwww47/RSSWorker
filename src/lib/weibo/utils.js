const weiboUtils = {
	...
	sinaimgTvax: (() => {
		...
		const replace = (html) => html.replace(regex, 'tvax');
		...
		const replaceKV = (obj, keys) => {
			keys.forEach((key) => {
				if (obj[key]) {
					...
					// 处理图片链接
					obj[key] = obj[key].replace(/(https?:\/\/[^"'<>\s]+\.(?:jpg|jpeg|gif|png|webp))/gi, addPrefix);
					// 处理视频链接
					obj[key] = obj[key].replace(/(https?:\/\/[^"'<>\s]+\.(?:mp4|m3u8)(?:[^"'<>\s]*)?)/gi, addPrefix);
				}
			});
		};
