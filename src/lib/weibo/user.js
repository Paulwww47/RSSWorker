	...
	let resultItems = await Promise.all(
		cards
			...
			.map(async (item) => {
				...
				// 添加这个函数来处理链接
				const addPrefix = (url) => {
					const prefix = 'https://953555d1.rsstt-img-relay-e85.pages.dev/';
				...
				// 处理描述中的链接
				description = description.replace(/(https?:\/\/[^\s"'<>]+)/g, addPrefix);
				...
				if (item.mblog.page_info && item.mblog.page_info.media_info) {
					const mediaInfo = item.mblog.page_info.media_info;
					['stream_url', 'stream_url_hd'].forEach(key => {
						if (mediaInfo[key]) {
							mediaInfo[key] = addPrefix(mediaInfo[key]);
						}
					});
				}
